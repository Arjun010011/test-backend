        ...
        <>
            ...
            <div className="relative min-h-screen overflow-hidden bg-background">
                <div className="pointer-events-none absolute inset-0">
                    ...
                    <div className="absolute bottom-0 right-0 h-72 w-72 translate-x-1/3 rounded-full bg-indigo-500/10 blur-3xl" />
                </div>
                            ...
                            {auth.user ? (
                                ...
                                <>
                                    ...
                                    {canRegister && (
                                        <Link
                                            href={register()}
                                            className="rounded-full border border-border bg-foreground px-4 py-2 text-xs font-semibold uppercase tracking-wide text-background transition hover:bg-foreground/90"
                                        >
                                            Create account
                                        </Link>
                                    )}
                    ...
                    <main className="grid gap-12 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
                        <div className="space-y-6">
                            ...
                            <h1 className="text-4xl font-semibold tracking-tight text-foreground sm:text-5xl">
                                A clean, focused space to manage your job search.
                            </h1>
                            <p className="max-w-xl text-sm leading-relaxed text-muted-foreground sm:text-base">
                                Upload your resume, surface your strongest skills, and stay ready for the roles that match your experience.
                            </p>
                            <div className="flex flex-wrap gap-3">
                                {auth.user ? (
                                    <Link
                                        href={dashboard()}
                                        className="rounded-full border border-border bg-foreground px-6 py-3 text-xs font-semibold uppercase tracking-wide text-background transition hover:bg-foreground/90"
                                    >
                                        Go to dashboard
                                    </Link>
                                ) : (
                                    <>
                                        ...
                                        {canRegister && (
                                            <Link
                                                href={register()}
                                                className="rounded-full border border-border/70 bg-background px-6 py-3 text-xs font-semibold uppercase tracking-wide text-foreground transition hover:border-border"
                                            >
                                                Create account
                                            </Link>
                                        )}
