...
class JavascriptCodingProblemProviderService
{
    /**
    ...
    public function generateProblemsFromBlueprint(array $questionBlueprint): array
    {
        ...
        foreach ($questionBlueprint as $selection) {
            ...
            if (! array_key_exists($topic, $availableTopics)) {
                throw new InvalidArgumentException('Selected topic is not available in the JavaScript coding dataset.');
            }
    ...
    private function problemCollection(): Collection
    {
        ...
        $path = (string) config('datasets.paths.coding.javascript', 'datasets/javascript_coding_problem_bank.json');

        if (! Storage::disk($disk)->exists($path)) {
            throw new InvalidArgumentException('JavaScript coding dataset file was not found.');
        }
        ...
        if (! is_array($payload) || ! isset($payload['problems']) || ! is_array($payload['problems'])) {
            throw new InvalidArgumentException('JavaScript coding dataset file is invalid.');
        }
